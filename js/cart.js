function displayCartItems() {
    ...
    cart.forEach(product => {
      ...
      const cartRow = `
        <tr>
          <td><img src="${product.image}" class="img-fluid me-5 rounded-circle" style="width: 80px; height: 80px;" alt="${product.name}"></td>
          <td>${product.name}</td>
          <td>$${product.price.toFixed(2)}</td>
          <td>
            <div class="input-group quantity" style="width: 120px;">
              <button class="btn btn-sm btn-minus bg-light border" data-id="${product.id}"><i class="fa fa-minus"></i></button>
              <input type="text" class="form-control form-control-sm text-center border-0" value="${product.quantity}" readonly>
              <button class="btn btn-sm btn-plus bg-light border" data-id="${product.id}"><i class="fa fa-plus"></i></button>
            </div>
          </td>
          <td>$${total}</td>
          <td><button class="btn btn-md rounded-circle bg-light border remove-btn" data-id="${product.id}">
    ...
    document.querySelectorAll('.remove-btn').forEach(btn => {
      btn.addEventListener('click', (e) => removeCartItem(e.target.closest('button').dataset.id));
    });
  }
...
function handleCheckout() {
    ...
    cartItems.forEach(item => {
        ...
        subtotal += itemTotal;
        orderMessage += `${item.name} - $${item.price.toFixed(2)} x ${item.quantity} = $${itemTotal.toFixed(2)}\n`;
    });
...
document.querySelector('.btn-proceed-checkout').addEventListener('click', handleCheckout);
